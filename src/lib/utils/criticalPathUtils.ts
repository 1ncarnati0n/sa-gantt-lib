...
// ============================================
// Critical Path 계산 유틸리티
// ============================================

/**
 * 특정 날짜가 휴일인지 확인 (특정 Task의 캘린더 설정 기준)
 */
const isHolidayForTask = (
    date: Date,
    holidays: Date[],
    ...
    // 토요일 체크
    if (!settings.workOnSaturdays && day === 6) return true;

    // 일요일 체크
    if (!settings.workOnSundays && day === 0) return true;
...
const getNetWorkValueForDate = (
    ...
    if (isHolidayForTask(date, holidays, settings)) return 0;
    ...
    while (currentDate.getTime() < d) {
        if (!isHolidayForTask(new Date(currentDate), holidays, settings)) {
            workDayIndex++;
        }
...
export const calculateCriticalPath = (
    ...
    for (let i = 0; i < totalDays; i++) {
        ...
        // 전역 캘린더 설정 기준 휴일 체크
        const isHolidayDay = isHolidayForTask(date, holidays, calendarSettings);
